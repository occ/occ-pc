...
}:
{
  programs.atuin = {
    ...
    settings = {
      ...
      # --- AI: disabled. The atuin CLI only speaks the Atuin protocol
      # (/api/cli/chat) to an atuin-ai-server shim, not LiteLLM/OpenAI
      # directly, and no shim is deployed. Disabling also drops the `?`
      # command-generation keybind.
      ai.enabled = false;
